    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            if (status === 404) {
                ...
                setErrors(prev => ({...prev, password: message || "Incorrect password"}));
            } else if (status === 403) {
                setErrors(prev => ({...prev, email: message || "Email not verified. Please verify your email first."}));
            } else {
                setErrors(prev => ({...prev, password: message || "Login failed. Please try again."}));
            }
